...
class ServiceNowAgent:
    ...
    async def retrieve_context(self, query: str) -> dict[str, Any]:
        ...
        if get_all_data:
            deployments = await self.tools.list_deployments_handler({"environment": "production"})
            incidents = await self.tools.list_incidents_handler({"query": ""})
            return {
                "source": "ServiceNow",
        ...
        if "deploy" in query_lower or "production" in query_lower:
            result = await self.tools.list_deployments_handler({"environment": "production"})
        ...
        if any(term in query_lower for term in ("all incidents", "list incidents", "show incidents")):
            result = await self.tools.list_incidents_handler({"query": ""})
        else:
            result = await self.tools.list_incidents_handler({"query": query})
